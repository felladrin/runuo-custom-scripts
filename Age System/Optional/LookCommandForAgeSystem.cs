...
//    > |      February 2010     | < 
//   /__|========================|__\	[Look Command for Age System] - Current version: 1.0 (February 10, 2010)
...
{ 
		...
		{ 
			...
			{ 
				if(from.Equals(targeted))
				{
					...
					from.Send( new DisplayProfile( !from.ProfileLocked, from, "Description of " + from.RawName, from.Profile, "You are " + ((Account)from.Account).GetTag("Age of " + (from.RawName)) + " years old.") );
				}
				else 
				{
	...
	public class lookGump : Gump
	{
		...
		public lookGump( Mobile m, Mobile target ) : base( 100, 100 )
		{
			...
			AddHtml( 20, 30, Width-40, Height-50, target.Profile + "<BR><BR>" + target.Name + " seems to be " + ((Account)target.Account).GetTag("Age of " + (target.RawName)) + " years old.", true, true );
		}
	}
}
